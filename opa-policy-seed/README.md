...
```bash
curl -s http://localhost:9181/v1/data/instruction/lifecycle/allow \
...
```bash
curl -s http://localhost:9181/v1/data/payment/lifecycle/allow \
  -H 'Content-Type: application/json' \
  -d '{
    "input": {
      "action": "CREATE",
...
OPA API (unauthenticated demo): http://localhost:9181
